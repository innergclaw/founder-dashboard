...
for (const marker of ["OWNYOURWEB", "INNERGINTEL", "SHOPNASGFX", "founder_projects", "founder_schedules", "founder_jobs", "founder_sync_runs", "founder_alerts", "founder_notification_preferences", "founder_brief_runs", "founder_daily_briefings", "founder_daily_runs", "founder_documents", "founder_applications", "founder_agent_items", "founder_agents", "founder_dispatches", "founder_calendar_events", "founder_calendar_sync_runs", "founder-dashboard-sync", "founder-daily-brief", "founder-chief-of-staff", "founder-dispatcher", "founder-research-analyst", "founder-web-builder"]) {
...
assert.ok(app.includes('"runs", "Daily Runs", "refresh"') && app.includes("What changed") && app.includes("What was decided") && app.includes("What failed or remains blocked") && app.includes("Where to resume"), "The private Daily Runs view must show the four founder report sections");
assert.ok(app.includes("Upcoming Cal.com calls") && app.includes("Cal.com checked at 7 AM and 7 PM"), "The twice-daily Cal.com call view must remain visible");
...
assert.ok(app.includes('"agents", "Agents", "layers"') && app.includes("Agent 2 · Dispatcher") && app.includes("Build every worker first. Connect one inbox second."), "Agent 2 and the mobile communication boundary must remain visible");
assert.ok(app.includes("Run research") && app.includes("Agent 3 report") && app.includes("worker-sources"), "Agent 3 must be runnable and its evidence must stay attached to the assignment");
assert.ok(app.includes("Prepare build brief") && app.includes("Agent 4 build brief") && app.includes("worker-evidence"), "Agent 4 must be runnable and its repository evidence must stay attached to the assignment");
